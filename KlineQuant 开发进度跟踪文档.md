...
| 2026-09-21 | v9.31 | MT5 实盘下单恒失败修复（FX 2 小时实盘测试启动即暴露：策略每次开仓 `OPEN LONG/SHORT: ok=False reason=[0] order_send returned None`，无任何真实成交）：根因是 v9.29 的 `_pipe_safe` 只做了顶层 `_asdict()`，而 `order_send` 返回的 `OrderSendResult` 内嵌 `request=TradeRequest(...)` 这一 C 层匿名 namedtuple 字段，子进程 `conn.send` 时 pickle 抛 `PicklingError: Can't pickle <class 'TradeRequest'>` → 走异常分支回传 `("err",...)` → 父进程 `_call` 返回 None → executor 误报“connection lost or timeout”；行情类调用（tick/copy_rates/symbol_info）无嵌套 C 对象故 v9.29 修复后一直正常，唯独交易路径 order_send 因带 request 嵌套字段触发（FX 订单系统 v2.0 实盘才首次走到）；实证：主进程直调 `mt5.order_send` retcode=10009 真实成交，`_pipe_safe(res)` 后 `pickle.dumps` 复现 PicklingError；修复：`_pipe_safe` 改递归展开——`_asdict`/dict/list/tuple 逐层递归转换，嵌套 namedtuple 全部落地为纯 dict（numpy 结构数组仍原样返回，copy_rates 路径不受影响）；测试：修正 test_mt5_source.py 陈旧导入路径（`_pipe_safe` 早已迁至 mt5_driver，旧路径致既有 test_pipe_safe_converts_unpicklable_namedtuple 一直 ImportError）+ 新增 test_pipe_safe_recursively_converts_nested_namedtuple（构造嵌套 OrderSendResult/TradeRequest 断言逐层转 dict 且可 pickle），全量 598 passed（另有 test_protocol.py 3 项 SymbolInfo 校验既有失败，与本次无关）；修复后重启 2 小时实盘测试首单即 `CLOSE SHORT/OPEN LONG: ok=True qty=0.01 price=1.14797`，持仓对账正常工作。附带排障：MT5 终端 AutoTrading 初始被关闭（retcode=10027），用户在终端开启算法交易按钮变绿后 trade_allowed=True。未提交 git |
| 2026-09-21 | v9.32 | MT5 对冲账户平仓误开新仓修复（v9.31 修复后用户发现终端出现 3 个持仓，暴露第二个更深的架构不匹配）：根因是该 IC Markets 账户为 **HEDGING 对冲模式**（同品种多笔独立持仓，positions_get 返回 3 条即为铁证），而 executor 平仓逻辑按 **NETTING 净持仓**设计——平仓只发反向市价 DEAL、**不带 position ticket**（原代码注释“close_ticket 仅 Hedging 账户需要，一期 Netting 不用”），对冲账户下“平空的 BUY”不会平空而是新开一个多单，故 v9.31 那笔 CLOSE SHORT + OPEN LONG 变成 1 空 + 2 多共 3 持仓；另一副作用：v9.31 序列化 bug 期间那些“ok=False”的下单其实已在终端真实成交（只是结果回传失败致系统误判），遗留幽灵空单 ticket=1947404931；处理：① 先按 ticket 逐个平掉 3 个孤儿持仓（对冲平仓=反向 DEAL + position=ticket + 等量），账户归零；② 修复 executor——submit() 将市价平仓路由到新增 _submit_close()，按 position ticket 精确平指定持仓（Netting/Hedging 均正确：净持仓模式 positions_get 返回单一净持仓带 ticket 同样按 ticket 平；对冲模式按开仓时间 FIFO 选与平仓方向相反的持仓逐笔平，可跨多笔，成交量加权均价聚合回报）；**关键安全**：找不到匹配持仓时返回 DEAD，绝不发裸反向单（杜绝误开新仓）；复用已预留的 VenueOrderSpec.close_ticket 钩子（显式指定则直按该 ticket 平）；③ 测试：新增 tests/unit/test_mt5_executor.py 7 用例（平空带 ticket/无匹配持仓拒单不发裸单/无持仓拒单/对冲 FIFO 跨多笔/忽略反向持仓/显式 close_ticket/开仓路径不受影响），全量 605 passed（test_protocol.py 3 项 SymbolInfo 校验为既有失败，与本次无关）；④ 重启 2 小时实盘测试干净启动首单 OPEN LONG ok=True、终端核实恰好 1 个持仓无重复。遗留：api.flatten() 仍按净敲算 net_vol 发单一 CLOSE（单方向持仓场景已由 _submit_close 兼底正确，若同时持多空两方向则需后续按 ticket 全平，非当前策略场景）；挂单类平仓（LIMIT/STOP CLOSE）未走 ticket 路径（策略仅用市价，待需要时补）。未提交 git |
| 2026-09-21 | v9.33 | fx_simple_test 策略改收盘价模型（用户指令：开平仓全用收盘价模型，收盘后条件成立才执行）：原策略用 api.is_changing(bars) 驱动，而 Mt5DataFeed._poll_bars 对未收盘 bar 的 close/high 每次变动都 bump 版本，致 is_changing 盘中每 tick 为 True、策略反复重算（实盘实测 DIF≈0、close 贴 EMA10 时信号几秒内频繁多空翻转，产生大量无效交易）；修复：改为时间戳驱动的收盘模型——跟踪 last_bar_ts，仅当 bars[-1].timestamp 变化（新 bar 出现=上一根收盘）时评估，盘中同一根未收盘 bar 直接 continue；指标用已收盘序列 closed=bars[:-1]（剔除仍在形成的最后一根，对齐项目既定“信号层收盘驱动、确认值读 iloc[-2]”约定），current_close=刚收盘那根收盘价；开/平仓均走同一信号判断故两者同为收盘价驱动。验证：重启后跨过一个完整 1m bar 收盘零交易零翻转日志（信号未变则 continue），对比旧模型每 ~6s 翻转；持仓恒为 1 个无堆积；启动对账同向不重复开仓。仅改 strategies/fx_simple_test.py（无单测覆盖的 demo 策略），全量回归仍 605 passed。未提交 git |
| 2026-09-21 | v9.34 | FX 实盘多品种改单进程多线程架构（用户指令：四品种为何开四进程，50 品种难道开 50 进程？→ 结束当前测试，直接改单进程多品种）：**推翻上一轮“每品种一进程”的权宜方案**（N 品种=N 进程=N 个 MT5 连接，不可扩展），改为项目既定决策（见“策略执行模型：单循环已推翻”记忆）的**单进程 + 每品种一工作线程**（品种间并行、品种内串行，规避单循环队头阻塞）。改动：① KqApi 由单 spec 改为 `specs: Dict[str,SymbolInfo]` + 新增 `symbols()`/`_spec_for(sym)`，`symbol_info`/`send_order` 按品种取各自 spec（不同品种 pip/step/digits 绝不混用），`log()` 加 `[STRATEGY:{symbol}]` 前缀便于多线程共享日志区分；② LiveRunner `symbol:str` → `symbols:Union[str,List[str]]`（兼容传 str），逐品种加载 spec、一个 feed 订阅全部品种、**每品种一个 KqApi 实例 + 一个 daemon 工作线程并行跑 strategy**（signal 处理留主线程，单品种异常只影响本品种不拖垮其他），逐品种对账、退出时逐品种 flatten 清仓；③ run_fx_live.py `--symbol` → `--symbols` 逗号分隔（保留 `--symbol` 别名）；④ fx_simple_test 策略**保持单品种不变**（各线程负各自 symbol，用 api 默认 symbol 回落）。测试：新增 tests/unit/test_kq_api_multi_symbol.py 6 用例（symbols()/按品种 spec/大小写回落/未知品种 KeyError/send_order 按品种路由 spec/账本按品种隔离），全量 **611 passed**（test_protocol.py 3 项 SymbolInfo 校验为既有失败，与本次无关）。验证：单进程启动日志 `(4 symbol(s))`+`4 symbol worker thread(s)`，4 线程并行、各自 spec（USDJPY pip=0.010 余 0.00010）、4 品种同时开仓 ok=True、终端恰好 4 持仓各 1。**注**：本次测试因“到 20:30”目标时刻已过（实际 22:54）算出负 duration→落入不限时，经用户确认改为立即停止，已按 ticket 对冲安全平掉 4 仓（retcode=10009，remaining=0）。遗留：run_fx_live 对负/零 duration 静默转不限时，宜加护栏；feed 的 wait_update 单 Event 多线程等待为电平触发（时间戳比对），延迟≤一个轮询间隔，50 品种时轮询需优化。未提交 git |
| 2026-09-21 | v9.35 | **回测/实盘同构 + MT5 历史数据回测**（用户指令：先实现回测/实盘同构，外汇回测直接用 MT5 历史数据）：目标是同一份 `strategy(api: KqApi)` 无需改动即可实盘/回测。核心设计——**KqApi/ExposureLedger/UnifiedResolver 零改动**，只替换数据源与执行器两个协议实现（依赖 DataFeedProtocol/ExecutorProtocol 注入）。新增 4 文件（strategy/sdk/）：① `backtest_feed.py` BacktestDataFeed（实现 DataFeedProtocol，逐 bar 回放历史，latest_bars 只返回到当前 index 的切片=**无未来数据**；open_price/close_price 供执行器取成交价与 mark-to-market；set_on_bar 注册逐 bar 回调）；② `backtest_executor.py` BacktestExecutor（@dataclass 实现 ExecutorProtocol，返回与 Mt5Executor **同一 SubmitResult**、query_positions/query_account 同形 MT5 dict，使 KqApi 零感知；撮合模型一期仅 MARKET（LIMIT/STOP 返 DEAD）、成交价=**当前 bar 开盘 ± 滑点**——策略用 closed=bars[:-1] 即信号 bar=index-1 收盘，成交落在 bar index 开盘=信号 bar 下一根开盘，**天然反 look-ahead** 与实盘语义一致；净持仓 Netting 记账对齐 resolver close_priority="net" 与策略「先平后开」；盈亏=价差×手数×contract_multiplier）；③ `backtest_runner.py` BacktestRunner（类比 LiveRunner，_load_from_mt5 用 Mt5Api.initialize+symbol_select+load_spec_from_mt5+copy_rates_from_pos 拉历史并**丢弃最后一根形成中 bar**，逐品种顺序跑同一 strategy_fn→末尾 api.flatten 清仓实现残留浮盈→PerformanceAnalyzer.analyze 出绩效；bars_by_symbol/specs 注入时跳过 MT5 供单测）；④ `scripts/run_fx_backtest.py` CLI（--symbols/--period/--strategy/--bars/--capital/--tag/--slippage/--fee/-v）。**关键 bug 修复（跨币种盈亏换算）**：首版实跑 USDJPY 报 return=-57.81%（其余 USD 品种仅 -0.2%~-0.5%），根因=USDJPY 盈亏以 JPY 计却当成账户币 USD（1 JPY≈0.0064 USD，放大 ~157 倍）；修复=新增 `_conv(sym, amount_quote, price)` 按 base/quote currency 换算到账户币（quote==USD 如 EURUSD 原样；base==USD 如 USDJPY ÷价格；交叉盘如 EURJPY 缺第三方汇率原样计并告警一次），应用到 submit 手续费/_apply_fill gross/_unrealized/query_positions profit，加 account_currency 字段与 _conv_warned。测试：新增 tests/unit/test_backtest_isomorphic.py **9 用例**（成交在 open 非 close/无未来 bar 可见/净持仓开平已实现盈亏/空头盈利/逐 bar 资金曲线 mark-to-market/flatten 清残留/Runner 端到端出报告/Runner 确定性/USDJPY 跨币种换算 1000 JPY÷156≈6.41 USD），全量 **620 passed**（test_protocol.py 3 项 SymbolInfo 校验为既有失败，与本次无关，零回归）。真实 MT5 端到端验证：EURUSD/GBPUSD/AUDUSD/USDJPY 各 1500 根 1m，return -0.19%/-0.51%/-0.30%/**-0.37%**（USDJPY 已从 -57.81% 修正至同量级，换算修复确认生效）。**与原计划偏差**：memory 25fff474 的 P1 原设计为 `core/backtest_engine/kq_adapter.py` + `BacktestKqApi`（另造回测专用 API），实际改为在 strategy/sdk/ 直接**复用真实 KqApi**（换 feed+executor 实现同构），同构度更高、零 API 重复。遗留：交叉盘无第三方汇率暂原样计（一期 4 品种不涉及）；equity_curve 有 ~1 bar 采样滞后（on_bar 在 wait_update advance 后、策略下单前触发，对指标影响可忽略）；一期执行器仅 MARKET 单；顺序回放非并行（品种独立故单品种结果无差异，未做多品种组合资金曲线）；bars_per_year 用 365 天约定（FX 实际 ~5 天/周，年化偏乐观）。未提交 git |
| 2026-09-21 | v9.36 | 清理 test_protocol.py 中 3 项长期失败的无效期望测试（用户问“3 个既有 test_protocol 失败是什么”→确认后按建议修复）：TestSymbolInfo::test_negative_price_precision / test_negative_qty_precision / test_zero_min_qty 均期望构造 SymbolInfo 时非法精度/零 min_qty 抛 ValueError，实际 DID NOT RAISE。根因：SymbolInfo 是纯数据 `@dataclass`（protocol/types.py）无任何 `__post_init__` 校验，spec_loader 也全程用 `.get(..., 0)` 宽容构造，整个项目取向为“纯数据载体、不在 dataclass 层校验”（TDD 遗留的无效期望）。**修复方向：删测试而非补校验**（保持 dataclass 纯净、避免 min_qty<=0 校验误伤 FX/回测大量默认构造路径）：删除这 3 个测试，保留 test_valid_symbol_info，并将类注释改为说明“构造期不校验、校验/容错职责在 spec_loader 等加载入口”。验证：全量 **620 passed、0 failed**（长期以来的 3 项既有失败清零）。附：该文件 ruff I001（import 块未排序）为改动前既有问题（HEAD 版本同样报）、与本次无关，未动。未提交 git |
| 2026-09-21 | 推送记录 | v9.31~v9.36 批量提交并推送（commit ab83405，8d40328..ab83405 main -> main，16 文件 +1849/-131）：MT5 子进程返回值序列化修复 + 对冲账户按 ticket 精确平仓 + fx_simple_test 收盘价模型 + FX 多品种单进程多线程架构 + 回测/实盘同构（MT5 历史数据 + 跨币种换算）+ test_protocol 无效期望测试清理。新增文件：strategy/sdk/backtest_{feed,executor,runner}.py、scripts/run_fx_backtest.py、tests/unit/test_{backtest_isomorphic,kq_api_multi_symbol,mt5_executor}.py。全量 620 passed / 0 failed。推送沿用一次性 -c 覆盖（http.sslBackend=schannel + credential.helper 重置为 manager + 代理 http://127.0.0.1:7897，全局配置未动；首次仅覆盖代理仍报 ca-bundle.crt 证书错，补 schannel 后端后成功）。里程碑标签：已在 HEAD（bd08a86）打附注标签 **v2.1.0**（“回测/实盘同构 + MT5 历史数据回测”，对应 v9.31~v9.36）并推送 origin（refs/tags/v2.1.0） |
| 2026-09-22 | v9.37 | 交叉盘盈亏换算修复（用户指出：MT5 规格自带换算信息，不必拉第三方历史汇率那么麻烦）：v9.35 的 _conv 对交叉盘（EURJPY，base/quote 均非账户币）只能告警原样计（放大 ~156 倍）。**实测发现 MT5 `order_calc_profit` 直接返回账户币利润**（内部走交叉汇率）：EURJPY 价差 1.0/vol 1.0 返回 635.56 = 100000 JPY ÷ 157.341（USDJPY），完全吻合，故 rate = profit / (价差×vol×mult) 即 quote→账户币 系数。实现：① mt5_driver 加 `order_calc_profit(action,symbol,volume,price_open,price_close)` 转发（子进程 getattr 动态分发，无需白名单，返回 float 可 pickle）；② backtest_runner 新增 `_load_conv_rate`：加载 spec 时对交叉盘（q≠acct 且 b≠acct）调一次 order_calc_profit 推导 rate 存 `conv_rates[sym]`（直盘返 None，_conv 用回测时点价更精确），新增 account_currency 参数；③ backtest_executor 加 `conv_rates` 字段，_conv 交叉盘分支改为乘 rate（无 rate 时兜底告警原样，保单测可注入）。测试：新增 test_cross_pair_uses_conv_rate_from_mt5（EURJPY 1000 JPY×0.0064=6.4 USD），全量 **621 passed / 0 failed**。真实 MT5 回测 EURJPY 1500 根 1m：cross-rate→USD=0.0063559、return=-0.31% final=9968.58（与直盘同量级，不再放大 156 倍、无告警）。近似说明：rate 用加载时点（当前）汇率而非历史时点，回测区间内 JPY/USD 波动小可接受。已提交推送（commit d5808a6，2a506c3..d5808a6 main -> main；本次推送未用任何 -c 覆盖，验证全局 git 配置修复生效） |
| 2026-09-22 | v9.38 | **前端 BacktestView 接 FX 同构回测 + FX 迭代计划**（用户指令：把遗留做迭代计划 + 先让回测在 UI 可用）：① 迭代计划文档新增「六·五、外汇（FX）交易与回测迭代计划」专项节（变更记录 v2.13）：归拢 FX 已完成核心闭环（FX-ORD/MT5/HEDGE/MULTI/CLOSE/BT/CONV）+ 当前迭代 FX-BT-UI + 遗留分 P1（FX-BT-LIMIT 挂单撮合/FX-FLATTEN 按 ticket 全平/FX-DUR-GUARD duration 护栏）与 P2（FX-BT-PORT 组合资金曲线/FX-BT-EQ 采样滞后/FX-BT-ANNUAL 年化/FX-CONV-HIST 交叉盘历史汇率/FX-CLOSE-LIMIT 挂单平仓 ticket/FX-FEED-SCALE 50 品种轮询）；② 后端新增 gateway/routers/backtest_fx.py（prefix /api/backtest/fx，复用 strategy/sdk BacktestRunner 跑 MT5 历史，run_in_executor 线程池异步）：GET /strategies（扫描 strategies/ 下含 def strategy 的模块，实测返回 fx_macd_demo/fx_simple_test）+ POST /run（symbols/period/strategy/history_bars/capital，对齐 BacktestRunner 真实签名 period/strategy_fn/history_bars/initial_capital/slippage_model+params/fee_model+params/magic/account_currency）+ GET /result/{task_id}（逐品种 report/equity_curve/trades/n_bars；profit_factor 等 inf/nan 经 _f 归 None 保 JSON 安全；trades 用 Trade round-trip 字段与旧路由同构）；app.py import+include_router backtest_fx（旧币安 dual_ma 路由保留不动）；③ 前端 BacktestView.vue 重写：顶部引擎切换 tab（FX 同构/币安 dual_ma），FX 模式表单（品种逗号分隔/周期/策略下拉/bars/资金）+ 2s 轮询 result（≤60 次）+ 多品种下拉切换 + metrics 网格 + 资金曲线 svg + 交易明细表（round-trip 列），币安模式原样保留。测试：新增 tests/unit/test_backtest_fx_router.py **14 用例**（策略扫描/加载、_f inf-nan、report/trade 序列化、fake runner 集成、失败标记 FAILED、端点 /strategies /run 空400 /result COMPLETED/PENDING/404），全量 **635 passed / 0 failed**。真实端到端：临时 uvicorn 8010，POST /run EURUSD 1m 300bars fx_simple_test → /result COMPLETED，return=-0.006% trades=11 final=9999.41 eqPts=301 pf=0.716（直盘正常量级，路由→真实 BacktestRunner→MT5→序列化全链路通）；vue-tsc --noEmit 对 BacktestView 无类型错误。遗留：FX 回测在 gateway 线程池起 MT5 子进程，与实盘 runner 同跑可能争用终端（一期不处理）；前端需 npm run build 后静态服务（8080）才更新，开发验证用 vite dev。已提交推送（见下方推送记录，commit b14584a） |
| 2026-09-23 | v9.39 | **加密（币安 Futures Demo）同构实盘模拟 + MarketBackend 抽象**（用户指令：现在只差加密的实盘模拟交易了 → 确认合约 Futures Demo + 通用 MarketBackend 抽象）：目标是让 crypto 复用同一份 `strategy(api: KqApi)` 同构架构跑实盘模拟（币安 USDT-M Futures Demo，One-way 净持仓），并把 LiveRunner 抽象为市场无关、为国内期货等新市场铺路，**FX 实盘行为零退化**。**阶段一（MarketBackend 抽象）**：① 新增 `strategy/sdk/backend.py`——`MarketBackend` Protocol（runtime_checkable：connect/load_specs/make_executor/make_feed/reconcile_positions/shutdown）+ `Mt5Backend`（逐行等价搬迁原 LiveRunner._initialize 的 MT5 全套逻辑：Mt5Api/load_spec_from_mt5/Mt5Executor/Mt5DataFeed/对账/driver.shutdown，构造参数 magic/deviation/mt5_kwargs）+ 模块级 `_reconcile_net_positions` 助手（MT5 同形 dict 净持仓对账，两 backend 共用）；② 重构 `live_runner.py`：`__init__(backend, symbols, period, strategy_fn, *, tag/poll_interval/bar_count/duration)`——移除 magic/deviation/mt5_kwargs（下沉 Mt5Backend），_initialize 全程委托 backend，删 _reconcile_positions，_shutdown 用 backend.shutdown，保留 ExposureLedger/UnifiedResolver/多品种各一 KqApi + daemon 线程模型 + flatten 清仓 + 优雅退出 + duration 时限不变；③ `run_fx_live.py` 构造 Mt5Backend 传入 LiveRunner，CLI 参数不变。回归 test_mt5_executor + test_backtest_isomorphic 17 passed 不破。**阶段二（币安三件套，新架构）**：④ `spec_loader.py` 新增 `load_spec_from_binance(info, symbol)`——从 exchangeInfo 的 filters 实测解析 LOT_SIZE(stepSize/minQty/maxQty)、PRICE_FILTER(tickSize)、MIN_NOTIONAL(notional)，market_type=FUTURES/qty_unit=COIN/contract_multiplier=1/can_short=True/close_priority=net/t_plus_n=0；⑤ 新增 `core/trade_engine/executors/binance_executor.py` BinanceExecutor（实现 ExecutorProtocol）：submit POST /fapi/v1/order（positionSide=BOTH，offset==CLOSE→reduceOnly=true，MARKET/LIMIT，FILLED→SubmitResult(FILLED,executedQty,avgPrice)/NEW→IN_FLIGHT/拒单→DEAD，order_ticket=orderId）、cancel DELETE、query_positions(GET /fapi/v2/positionRisk，positionAmt 符号→type 0/1)、query_account(GET /fapi/v2/balance USDT)、query_orders(GET /fapi/v1/openOrders) 全部转 **MT5 同形 dict**（BacktestExecutor 已验证此模式使 KqApi 零改动）；**核心技术约束**：ExecutorProtocol 同步而币安 REST 为 async，故 backend 持独立 event loop 线程，所有 async 调用经 `run_coroutine_threadsafe(coro, loop).result(timeout=15)` 桥接为同步；⑥ 新增 `strategy/sdk/binance_feed.py` BinanceDataFeed（实现 DataFeedProtocol）：复用 BinanceFuturesAdapter，start() 走 REST fetch_klines 预热历史 bars + subscribe_kline(WS 回调) + start_ws（均在 loop 线程），WS K 线回调更新 bars 缓存（新 bar append/未收盘 bar 更新 close-high-low）+ bump version + threading.Event.set，bars dict 与 Mt5DataFeed._row_to_bar 同形，latest_bars/latest_tick/wait_update/is_changing/now_ms 与 Mt5DataFeed 同机制（Lock + Event 电平触发 + 版本快照）；⑦ `backend.py` 新增 `BinanceBackend`：后台 daemon 线程跑 asyncio loop(run_forever)，connect() 在 loop 上 adapter.connect + 建 httpx client + 设 One-way(POST /fapi/v1/positionSide/dual=false) + set_leverage，load_specs 走 GET /fapi/v1/exchangeInfo→load_spec_from_binance，make_executor/make_feed 注入 loop，reconcile 复用净持仓助手，shutdown 走 adapter.disconnect + client.aclose + call_soon_threadsafe(loop.stop) + join（币安专有模块懒导入，FX-only 路径不受影响）。**阶段三**：⑧ 新增 `scripts/run_crypto_live.py`（CLI --symbols/--period/--strategy/--duration/--leverage/--bars，从 .env 读 demo-fapi 配置，动态加载 strategies.{name}.strategy，banner 对标 run_fx_live）；⑨ `.env.example` 加币安 Futures Demo 配置（BINANCE_FUTURES_REST_BASE=demo-fapi/WS_BASE=demo-fstream/API_KEY/API_SECRET/BINANCE_TESTNET）。测试：新增 **4 测试文件共 34 用例**——test_spec_loader_binance.py(5，filters 解析/FUTURES 元数据/精度兜底/非交易 SUSPENDED/minNotional 旧字段)、test_binance_executor.py(15，真实 loop 线程 + AsyncMock httpx 验证 submit MARKET-OPEN→FILLED/CLOSE→reduceOnly/LIMIT→IN_FLIGHT/拒单→DEAD/不支持 kind→DEAD、cancel、query_* 返 MT5 同形 dict、同步桥接)、test_binance_feed.py(8，假 adapter WS 回调→bars 缓存/append/未收盘更新/切片/bar_count 裁剪/wait_update 唤醒/is_changing/stop 唤醒)、test_market_backend.py(6，Mt5Backend mock driver 注入 LiveRunner 全流程验证委托 + 多品种各一 KqApi + 退出 flatten 平净仓 + reconcile 回写 ledger；BinanceBackend 真实 loop + mock 验证协议符合/load_specs/make_executor-feed/reconcile/connect 设 One-way+杠杆/shutdown 关闭 adapter-client-loop），全量 **715 passed / 0 failed**（零回归）。遗留（待用户手动验证）：run_crypto_live 连 demo-fapi 实跑 fx_simple_test BTCUSDT 1m 真实下单；若 demo 账户锁定 Hedge 模式需 positionSide=LONG/SHORT 适配（connect 已显式设 dual=false）；BinanceExecutor submit FILLED 依赖响应 avgPrice（futures MARKET 单会回传，若为 0 则 ledger 均价不更新）；代理 7897（REST+WS 均需）。已提交推送（见下方推送记录，commit b14584a） |
| 2026-09-23 | 推送记录 | v9.38~v9.39 批量提交并推送（commit b14584a，6e09f1a..b14584a main -> main，18 文件 +2807/-215）：前端 BacktestView 接 FX 同构回测 + FX 迭代计划（backtest_fx 路由 + 前端引擎切换 tab/轮询/资金曲线）+ 加密（币安 USDT-M Futures Demo）同构实盘模拟 + MarketBackend 抽象（Mt5Backend/BinanceBackend + BinanceExecutor/BinanceDataFeed + run_crypto_live）。新增文件：gateway/routers/backtest_fx.py、strategy/sdk/{backend,binance_feed}.py、core/trade_engine/executors/binance_executor.py、scripts/run_crypto_live.py、tests/unit/test_{backtest_fx_router,binance_executor,binance_feed,market_backend,spec_loader_binance}.py。全量 715 passed / 0 failed。本次推送未用任何 -c 覆盖（全局 git 配置沿用 v9.37 修复：http.sslBackend=schannel + credential.helper=manager + 代理 7897）。版本标签：已在 HEAD（8e53c72）打附注标签 **v2.1.1**（对应内部 v9.37~v9.39：交易盘盈亏换算 + FX 同构回测 UI + 加密(币安 Futures Demo)多市场实盘模拟 + MarketBackend 抽象）并推送 origin（refs/tags/v2.1.1） |
| 2026-09-23 | v9.40 | **态① web 栈一键启动 + gateway 托管 Vue 正式版前端**（用户定案运行模式三态后落地态①：纯看板/回测无策略时独立一键启动，解决“回测手动拉起”痛点；三态详见迭代计划六·六节）：① gateway 托管 Vue dist——app.py 新增 DIST_DIR + `/app` SPA 服务（`/app/assets` StaticFiles 挂载 + `/app/{rest:path}` history 深链回退 index.html + `DIST_DIR+os.sep` 目录穿越防护），dist 未构建时静默跳过不影响 `/` 与 API；`/` → lc-live.html 基线一字未动（307→/static/lc-live.html，139077B 与直接访问一致）；② 前端 base 条件化——vite.config.ts `base: command==='build'?'/app/':'/'`（`npm run dev` :5173/ 工作流零变化），router `createWebHistory(import.meta.env.BASE_URL)` 跟随；③ 新增共享 web 栈启动器 `klinequant/webstack.py`（137 行）——`run_foreground()`（态①前台起 gateway + 轮询 /api/system/health 就绪后自动开浏览器）+ `start_in_background()`（态③ WEBGUI-L1 预留：`_NoSignalServer` 覆写 install_signal_handlers 绕过 uvicorn 非主线程注册 signal 限制，后台 daemon 线程跑 gateway）+ `python -m webstack` 入口（--host/--port/--no-browser/--open）；④ 新增态① 一键脚本 `start_klinequant.bat`（工作区根，双击：dist 缺失自动 npm run build 兜底 → 起 webstack → 开浏览器；Ctrl+C 单进程停止，不再需 vite preview :8080）；⑤ 构建 npm run build（vue-tsc 类型检查通过，87 模块，base=/app/ 生效：index.html 资源指向 /app/assets/ 与 /app/favicon.svg）。冒烟测试（临时 :8010）5/5 通过：/api/system/health 200 healthy engines=6、/app/ 200（hasApp+hasAssets）、/app/backtest 深链回退 200、/app/assets/index-*.js 200（ctype text/javascript、46304B 与构建一致）、/ 307→lc-live.html 200。**同期文档维护**：迭代计划删除已放弃的 INT-CLOSED（前端不交易、“接前端基线”失效）+ 更正 v1.1.0 状态（交易闭环已在策略层跑通）；需求文档五层+ZMQ 架构经核实为真实代码（core/*_engine ZMQ PUB/SUB/REQ-REP + config.ZmqConfig，为可选传输，本地默认 gateway 单进程内聚合）保留架构，另在需求文档 §3.1 补一句实现现状注解（本地默认 gateway 单进程内聚合、ZMQ 为可选/远程拆分传输）。未提交 git |
| 2026-09-23 | v9.41 | **A 股切所请求 K 线恒空修复（“✕ 未返回 K 线数据”）**（用户报：A 股品种列表有数据，但请求 K 线时前端右下角提示“✕ 未返回 K 线数据（数据源可能暂不可用…）”）：**根因**——lc-live.html 周期工具栏为固定档（1m…4h…1Y），设计上 `applyTfAvailability` 因 `tfValid()` 逃生舱对固定档一律放行、从不禁用；但 ths（A 股）原生只支持 1m/5m/15m/30m/1h/1d/1w，网关派生层（derived.py）只聚合**日级以上**（Nd/Nw/NM/1Q/1Y），`4h` 在 NATIVE_TIMEFRAMES 中被判为“原生档”，故 router 守卫 `timeframe not in source.supported_timeframes` 直接返回 count=0。用户在 binance 常看 4h，点 A 股品种触发 `switchExchange`，其回退条件 `!meta.timeframes.includes(curTf) && !tfValid(curTf)` 因 `tfValid('4h')===true` 恒不成立→保留 4h→`load()` 拿到空数据→报错。实证：对运行中后端直请 `exchange=ths`，`4h/2h/12h/3d → count=0`，而 `15m/1d/1w/1M/1m → 正常返回`。**修复（前端 lc-live.html，以后端 meta.timeframes 为唯一真值）**：① 新增 `tfAvailable(tf,meta)`（自定义倍率恒可用；固定档须在 meta.timeframes 内）+ `fallbackTf(tf,meta)`（优先取不大于当前档的最大周期，如 ths 4h→1h；都更大时取最小档）+ `TF_DURATION` 时长表；② `applyTfAvailability` 改用 `tfAvailable` → ths 上 4h 按钮真正置灰禁点；③ `switchExchange` 回退条件改 `!tfAvailable(curTf,meta)` → 切 A 股时 4h 自动回退 1h；④ `initSources` 恢复态同样回退（上次 binance 用 4h、本次恢复 exchange=ths 时防首屏空数据），回退结果 saveLayout 持久化。附带修正 base.py `meta()` 文档串（原称“派生层让所有源拥有全部周期、前端按钮不再逐源禁用”与实际不符——日内档 ths 无原生供给且派生层不重采样日内，改为说明前端以 meta.timeframes 逐档禁用+回退）。**语义正确性**：A 股连续竞价仅 4 小时/日（09:30~11:30、13:00~15:00），4h/2h/6h/12h 档对 A 股本就无意义，禁用非权宜而是正解。测试：test_ths_source + test_derived_tf 共 38 passed（后端仅改注释无功能影响）；前端内联脚本经 node `new Function` 语法校验通过、tfAvailable/fallbackTf 逻辑经 node 独立断言（ths 4h 不可用→回退 1h、2d/3w 自定义倍率可用、binance 4h 可用、无 meta 放行）。前端改动待用户人工验证。未提交 git |
| 2026-09-23 | v9.42 | **MT5 K 线中段缺口自愈修复（XAUUSD 蜡烛图空缺但指标线相连）**（用户报：XAUUSD 5m 周期 19:40~21:35、1m 周期 19:38~21:18 出现 K 线丢失、无蜡烛但有指标）：**根因**——`kline_cache` 的缓存条目一旦出现**中段空洞**便永久无法自愈：命中最新端时只做 `_TAIL_REFRESH=3` 根尾部刷新、缺口补拉只朝更早方向（`anchor=bars[0]-1` 前向），两者都无法回填中段。本次触发链：MT5 驱动子进程（PID 22340）于缺口末端时刻重启（实时流 `stream_loop` 只轮询**最新 1 根**，掉线期间 11:37~13:19 rawUTC 的 bar 全部漏采），恢复后尾刷把最新几根追加到缓存末尾（末根仍停在 11:37），中段 101 根 1m/18 根 5m 缺口就此焊死。前端表现：蜡烛图在缺口处空缺、而指标 line series 直连缺口两端相邻点→“有指标没 K 线”。**实证定位**：① 对运行中后端直请 `exchange=mt5`，XAUUSD 1m 缺口 11:37→13:19 rawUTC（≈101 根）、5m 缺口 11:35→13:10；② **XAUUSD 独有**——EURUSD/GBPUSD 同窗口连续（终端与网关健康、数据在）；③ 未缓存的 XAUUSD 15m/30m/1h 强制全量新拉：1h 全程连续（仅日终 23:00→01:00 合法休市缺口）、15m 仅 11:30→12:00 一处窄洞→证明 MT5 终端重连后已回补 12:00→13:19 数据，仅 ~11:45 附近为**源侧真实缺数据**（任何代码都无法填）。**修复（`gateway/market_sources/kline_cache.py`，后端根因）**：新增中段空洞自愈——命中最新端时在**将返回的服务窗口**内扫描相邻间隔 > `interval×_GAP_TOL(1.5)` 的可疑空洞，向源按范围补拉（`_fetch_page(end_time=b-1)`）并按时间戳去重合入缓存；仅对**日内原生档**（`_interval_ms` 解析且 `<_DAY_MS`；派生档 1M/1Q/1Y/Nd… 返回 None、日/周线周末缺口属正常结构不触发）生效；每缺洞冷却 `_GAP_RETRY_COOLDOWN=120s` 内只试一次（源返空的合法休市/周末缺口不反复拉取）、单次请求最多补 `_GAP_REPAIR_MAX=3` 处、`_Entry` 增 `gap_attempts` 冷却台账（>512 兜底清空）。**部署说明**：gateway 以 `uvicorn.run` 无 reload 启动，需**重启**才加载新代码；重启同时清空进程缓存→当前 XAUUSD 缺口立即随全新建底愈合（除源侧真实缺失的 ~11:38~11:59 外）；此后源掉线遗留的中段缺口无需重启即自愈。测试：`test_kline_cache` **9 passed**（新增 3：中段缺口从源回填恢复连续 / 合法空洞源返空后冷却内二次请求不再拉源 / `_interval_ms` 解析与派生档·日线门禁），market 相关全量 **164 passed / 0 failed**（零回归）；ruff 新增代码零告警（文件仅余 3 处 HEAD 既存 E501/UP037，非本次引入，未动）。未提交 git |
| 2026-09-23 | v9.43 | **阶梯线渲染能力（Pine `plot.style_stepline` 前端还原）+ `style.step` 契约扩展**（用户需求：TradingView 阶梯线能否实现 → 方案论证后执行方案三）：lwc v5.2.0 无原生阶梯线型；方案二（数据插点）因 lwc 索引制时间轴会被插点占槽、破坏蜡烛布局而否决，选定方案三（Series Primitives API 自绘）。① 后端 `dsl.py` 契约扩展：`_validate_style` 支持 `step` 键（布尔校验、仅 `line` 字段生效、`true` 归一化输出该键 / `false`·缺省不产键），`@pyindicator` docstring + 契约注释同步；② 前端 `lc-live.html` 新增 `StepLinePrimitive`：挂载字段的 LineSeries 线体设 `transparent` 仅承载数据（autoscale/十字线图例/历史 setData/实时 update/切周期清空全部照常、零额外同步代码），primitive `draw` 时直读 `series.data()` 自绘「水平段+垂直跳变」，二分定位可见区（`getVisibleRange` + 两端各多取一点保首段水平线），`useBitmapCoordinateSpace` 像素比对齐（奇数线宽半像素保 1px 清晰），`STEP_DASH` 线型映射（lwc LineStyle 1 点线/2 虚线/3 大虚线 → canvas `setLineDash`）；`buildInstance` custom 分支按 `bs?.step===true` 挂 primitive 存 `inst.stepPrims[i]`，`applyInstStyles` 将用户自选颜色/线型转发到 primitive 就地生效（series 保持透明），图例色仍用可见色 `st.color`；③ 首个使用方（属指标迁移，按规则不单独成条）：`mr_y_main`（MR_Y_MAIN 主图均线系统 17 字段中 11 个 EMA/布林通道字段打 `step=True`，SMA 字段保持普通实线）+ `mr_y_cycle`（MR_Y_CYCLE 副图多倍数随机指标族 12 字段全 `step=True`），二者系 Pine「Mister.Y MainChart」/「Mister.Y cycle」v5 迁移，批量 vs 增量一致性（8.2e-13 / 9.2e-13）+ 末根纯 Python 手算对照（≤3.4e-13）通过；④ 测试：`test_graph_indicator.py` 扩展 `test_style_contract_validation`（step True 归一化 / False 不产键 / `step:1` 抛错 / `histogram+step` 抛错）+ 新增 `test_step_style_in_custom_indicator_meta`，指标单测 63→**64 passed** 零回归；ruff 经 HEAD 对照确认 47 处告警全为既存债务、本次零新增；前端内联脚本 node `new Function` 语法校验 + 关键符号存在性检查全过；⑤ 部署：gateway `uvicorn` 无 reload，新指标与 dsl 契约需**重启** gateway 生效，前端刷新即可；⑥ **首屏联调修复（关键陷阱）**：用户重启后刷新出现「K线与全部指标黑屏，但品种列表/状态栏 DOM 与 WS 正常」——根因是 StepLinePrimitive 的 paneView 把 `renderer` 误写为属性对象 `{renderer:{draw}}`，而 lwc v5.2.0 `IPrimitivePaneView.renderer` 是**方法**（`renderer(): IPrimitivePaneRenderer|null`，查 node_modules/lightweight-charts/dist/typings.d.ts 确认），lwc 调 `view.renderer()` 抛「not a function」**每帧中断整个 canvas 渲染帧**（故蜡烛与全部指标一起黑、DOM/WS 独立不受影响）；该错在 lwc 调 renderer() 时触发、位于 draw 内部 try/catch 之外故止不住，且被 load() 兑底文案误报为「后端未连接」（首次撞上后端暖启动窗口）。修复：`renderer` 改为方法 `() => this._renderer` + constructor 缓存 renderer/paneViews 数组（lwc 基于数组引用缓存，paneViews() 须返回同一数组）+ draw 包 try/catch（单 primitive 异常不拖垮全图、首次 console.error 暴露真实堆栈）；scope 属性（fancy-canvas BitmapCoordinatesRenderingScope：context/horizontalPixelRatio/verticalPixelRatio）经 typings 核对正确。node 语法校验通过。遗留：待用户人工验证（阶梯观感 / 样式弹窗改色改线型即时生效 / 蜡烛布局无变化）；MR_Y_CYCLE 原版 `show_max=false`（1024x 组默认隐藏）迁移后常显（def 式不支持布尔显隐开关）。未提交 git |
| 2026-09-23 | v9.44 | **网关静态资源缓存策略：`/static` 挂载加 `Cache-Control: no-cache`**（v9.43 阶梯线黑屏事故收尾：用户浏览器命中旧 lc-live.html 的直接成因是响应无显式缓存策略、浏览器按启发式新鲜度普通导航复用旧缓存）：`app.py` 新增 `_NoCacheStaticFiles(StaticFiles)` 覆写 `file_response` 统一附加 `Cache-Control: no-cache`——语义为「允许存储但每次使用前 ETag 重新校验」，未变服务器回 304 不传正文，代价仅每次开页一次毫秒级本地校验（≠ no-store，不禁用缓存）；作用域仅 `/static` 挂载（lc-live.html/lwc.js 等静态文件），`/app/assets`（Vue dist）与各 `/api` 响应不受影响（TestClient 进程内实证：/static 命中 no-cache+ETag、/api/system/health 无该头）；与后端指标计算缓存（engine `_kline_cache`/`_series` 契约缓存）、进程级 K 线缓存（`kline_cache.cached_klines`）、页内渲染快照完全无关，切周期/品种速度零影响。验证：TestClient 响应头作用域断言 + py_compile；ruff 经 HEAD 对照零新增（较 HEAD 多的 2 处 E501 为工作区 Vue dist 挂载既存未提交改动，非本次引入）。部署：gateway 无 reload 需**重启**生效；生效后普通刷新即恒取最新前端，无需 `?v=2` 后缀或手动硬刷新。未提交 git |
| 2026-09-23 | v9.45 | **指标设置弹窗限高 + 内部滚动（TradingView 式）**（用户报：指标内参数较多时样式设置框占据整列屏幕、底部确认/取消按钮无法点击）：`lc-live.html` 将 `#ind-fields` + `#ind-style-wrap` 包入新增 `.ind-modal-body` 滚动容器，`#ind-modal .modal` 改 flex column + `max-height:min(72vh,660px)`——head/页签/按钮区固定、仅中部 body `overflow-y:auto`（滚动条复用全局 6px 样式）；max-height 为上限故内容少的弹窗（如 RSI）仍紧凑；添加指标面板 `#ind-pick-list` 同族防御限高 `max-height:52vh` 滚动，自定义指标增多时底部关闭按钮保持可见。纯 HTML/CSS 改动零 JS 改动（indTabTo 页签切换 / openIndModal innerHTML / 色板委托绑 `#ind-style-wrap` 均按 id 或后代选择器访问，不受包裹层影响）。内联脚本 node 语法校验通过。待用户人工验证。未提交 git |
| 2026-09-23 | v9.46 | **阶梯线指标历史段不绘制修复（预热加深迟滞锁死缺口）**（用户报：刷新页面后使用阶梯线的指标——MR_Y_MAIN/MR_Y_CYCLE——历史部分不绘制，只绘最近约 250 根）：**根因**——`indicator_service.ensure_warmed` 的预热加深迟滞 `if indicator.is_warmed_up and need - series_len < _PAGE_FETCH_LIMIT(1000): return`，会把一次**瞬时短拉**（`kline_cache` 补页异常 `break` 或短页）留下的 <1000 根缺口**永久锁死**：引擎有效序列长度恒为 `已预热根数 − min_periods`，一旦与前端显示需求 `need`（=klines.length）的差距落入 (0,1000) 区间，此后每次历史请求都命中迟滞直接返回短序列、再不加深。**实证定位**：① 直请运行中后端 `limit=1000/6000/9000` 均返回满量（BTCUSDT 15m 源深达 9000+），证明**非源尽头**、亦非 `min_periods` 虚高（MR_Y_MAIN 最长链 `sma(close,764)`、MR_Y_CYCLE `bet_1024x=768` 结构性预热，不可压缩）；② 浏览器子代理复现刷新态：`klines=6000` 而 MR_Y_CYCLE 全 12 系列恒 `5258 = 缓存 6026 − 预热 768`，`diff=742<1000` 恰落迟滞区间→永久跳过；`limit=9000` 直请因 `diff=3742≥1000` 绕过迟滞才返回满量，反证迟滞即元凶；③ 用户观感「约 250 根」= 源较浅品种 `1000 首屏 − 768 预热 = 232`。**修复（`gateway/indicator_service.py`，后端根因）**：删除 `_PAGE_FETCH_LIMIT` 迟滞分支，改为**只要 `series_len < need`（存在左侧缺口）就按 `need+min_periods` 加深预热**；`series_len >= need`（完整覆盖）仍即时返回免重算（切周期/品种热路径不变）。安全性：实时 bar 走 `on_bar`/WS 增量 `series.update`，**不经** `ensure_warmed`（`renderChart`/`fetchIndSeries` 仅 load/loadMore/换色/增删指标时触发，且 `loadingMore` 串行化），故移除迟滞不会引发逐 bar 全量重算；`cached_klines` 命中存量时仅尾刷 3 根+切片开销极小，源确已到头时 `warmup` 幂等无副作用。测试：新增回归 `test_history_deepens_within_old_hysteresis`（源 600 根，limit 300→500 diff 200<旧阈值 1000，断言仍加深至 count=500 且 fetch_calls 增加；旧代码此处会跳过返回 300 而 FAIL），`test_indicator_gateway.py` **9 passed**（原 8 + 新 1）、`test_graph_indicator.py` 联跑 **20 passed** 零回归；ruff 经 HEAD 对照 `indicator_service.py` 与测试文件均零新增（既存 UP035/UP045/UP006 及测试 2 处告警为 HEAD 债务，仅随插行位移）。**部署说明**：gateway `uvicorn` 无 reload，需**重启**加载新代码；重启清空进程缓存与引擎序列，刷新后首次历史请求即按 `need+min_periods` 建满深度，阶梯线历史段完整绘制。未提交 git |
| 2026-09-23 | v9.47 | **图例参数行超宽省略号截断（不再跨出主图边界）**（用户报：指标字段过多时——如 MR_Y_MAIN 17 字段——图上参数行会跨出主图右边界）：**根因**——`lc-live.html` 的 `.legend-row` 原为 `display:flex + flex-wrap:wrap + width:max-content`，而主图指标图例片段 `.ind-main-wrap`（JS 置 `display:inline-flex`）是**原子 inline-flex 块**：既不换行、作为 flex 项又不收缩（min-width:auto），字段一多整行溢出主图右边界。**修复（纯 HTML/CSS + 1 行 JS，零业务逻辑改动）**：`.legend-row` 改 `display:block + white-space:nowrap + overflow:hidden + text-overflow:ellipsis + max-width:100%`（删 `width:max-content`/`flex-wrap`/`gap`），宽度由父容器 `.legend`（left/right:12px）明确界定→超宽部分渲染 …；行内片段间距用 `.legend-row > * { margin-right:10px }` 与 `.ind-main-wrap > * { margin-right:10px }` 替代原 flex gap:10px（末子 margin:0），齿轮 `vertical-align:middle` 对齐；JS `buildInstance` 主图分支 `wrap.style.display` 由 `inline-flex` 改 `inline`（随 .legend-row 一起 nowrap 截断，不再整块溢出）。副图图例行（.legend-row）同机制受益：由多行换行改为单行省略号，更紧凑不遮挡指标。node 内联脚本语法校验通过。待用户人工验证（多字段主图指标图例单行且右侧 … / 齿轮可点 / 悬停冻结不闪烁 / 副图图例不超宽）。未提交 git |
| 2026-09-23 | v9.48 | **主图多指标图例改为每指标独占一行（不再挤同一行互遮）**（用户报：多个指标放同一行时，靠后的指标参数被省略号吃掉看不到）：v9.47 将图例行改单行省略号后，主图多个指标仍共用一行 `#lg-main-row`（各 `.ind-main-wrap` 内联拼接），第一个指标占满宽后其余被 … 截断。**修复（纯 HTML/CSS + JS 建行人造，零业务逻辑改动）**：删除 `#lg-main-row` 共用行，`#lg-main-items` 由行内 span 改为 `.legend`（flex column）直接子元素 `div` + `#lg-main-items { display:flex; flex-direction:column; gap:4px }`；`buildInstance` 主图分支不再建 `.ind-main-wrap` 内联块，改为每个指标建一个 `.legend-row` div（与副图行同构，`data-ind-id` 标记）append 到 `#lg-main-items`→纵向堆叠、各自 nowrap 省略号截断互不遮挡；`inst.legendEl` 指向该行 div（destroyInstance `.remove()` / 悬停冻结 rowEl / 齿轮查询均兼容）；VOL 从原 `#lg-main-row` 上提合并到 OHLC 行尾（`#lg-vol` id 不变，显隐切换与 setLg 赋值照常）；rebuildIndicators 清理仍用 `#lg-main-items.innerHTML=''`（保留 `#lg-ohlc`）。同步删除 v9.47 遗留的 `.ind-main-wrap` CSS 规则（已无引用）。node 内联脚本语法校验通过。待用户人工验证（多个主图指标各占一行且各自右侧 … / 齿轮可点 / VOL 在 OHLC 行尾正常显隐 / 副图行不变）。未提交 git |
| 2026-09-23 | v9.49 | **图例右边界让开价格刻度区（不再压右侧数值栏）**（用户报：参数行边界还是太大、影响到右侧数值栏）：v9.47/v9.48 的 `.legend` 仍用 `right:12px`，而 `#chart` inset:0 含右侧价格刻度区，故图例（及省略号截断点）会横跨并遮挡最右侧价格数值栏。**修复（纯 CSS + layoutPaneLegends 一处，零业务逻辑改动）**：`.legend` 的 `right` 改为 CSS 变量 `right:var(--legend-right, 12px)`；`layoutPaneLegends`（已每 300ms 定时运行）读取实时价格轴宽 `chart.priceScale('right').width() || 60`，将 `--legend-right` 设为 `轴宽+8px` 写在 `.chart-wrap`（`container.parentElement`）上——CSS 变量向下继承，主图与副图所有 `.legend` 统一收口，省略号截断点落在价格轴左缘之前；轴宽随价格位数/成交量窗格变化时下一个 300ms 周期自适应（仅变化时写入，`__lastLR` 去抖）。node 内联脚本语法校验通过。待用户人工验证（图例及 … 不再进入右侧价格数值栏 / 切换高位价品种或开关成交量窗格后图例右边界随轴宽自适应 / 副图图例同样不超宽）。未提交 git |
| 2026-09-23 | v9.50 | **图例数值去千分位逗号（OHLC + 指标字段）**（用户问：参数的数值为什么会在千位出现逗号）：**根因**——`refreshLegend` 中 OHLC 与各指标字段值均用共用格式化器 `fmt = v => v.toLocaleString('en-US', {min/maxFractionDigits:curPrec})`，`en-US` 默认 `useGrouping:true` 会插入千分位逗号，故大价位指标（如 MR_Y_MAIN 在 BTC 上值≈价格）图例显示 `60,000.00`（与旧设计意图“指标值 toFixed 防逗号致宽突变”相左）。**修复（纯 JS，新增一个格式化器 + 改图例调用点）**：新增 `fmtVal = v => v.toLocaleString('en-US', { useGrouping:false, min/maxFractionDigits:curPrec })`（与 fmt 只差 useGrouping），`refreshLegend` 的 OHLC 4 行（lg-o/h/l/c）与两处指标值（十字线取值分支 + 末根 bar 分支）均改用 `fmtVal`→图例内 OHLC 与指标值一致无逗号；**顶部 ticker（最新价/24h 高低）与品种列表价格（fmtSym 兑底）仍用带分组的 fmt**（那里大价位加逗号更易读、用户未投诉）。node 内联脚本语法校验通过。待用户人工验证（图例指标值与 OHLC 不再出现千分位逗号 / 小数位数与轴一致 / 顶部 ticker 与品种列表价格仍带逗号 / 十字线取值与悬停冻结不闪烁）。未提交 git |
| 2026-09-23 | v9.51 | **图例字段参数名改为悬停提示（不再常驻显示）**（用户要：参数名不用固定显示，改为鼠标移到数值上显示对应参数名）：原 `buildInstance` 图例行每个字段为 `<span style="color:…">LABEL <b>value</b></span>`（LABEL 常驻），字段多时图例很宽。**修复（纯 JS，rowHtml 一行）**：去掉常驻 `${p.label} ` 文本，改为将参数名放到包裹 span 的 `title` 属性→`<span style="color:…" title="${p.label}"><b>value</b></span>`，鼠标悬停数值即原生 tooltip 显示参数名；指标名称（lg-label）与齿轮保留常驻。为何用原生 title 而非 CSS 伪元素提示：`.legend-row` 为省略号截断设了 `overflow:hidden`，会裁掉行内 CSS 提示，而原生 title 由 OS 渲染不受裁剪；且悬停时命中已有图例冻结逻辑（pointerenter→legendFrozen），读提示时数值不抽摅。覆盖全部指标类型（MA/BOLL/MACD/KD/RSI 及 custom，parts 均携 label）。node 内联脚本语法校验通过。待用户人工验证（图例只剩颜色区分的数值、悬停某值弹出对应参数名 / 主副图均生效 / 齿轮与指标名仍常驻 / 行宽变窄后省略号减少）。未提交 git |
| 2026-09-23 | v9.52 | **图例指标名只显基础名、括号参数改悬停提示**（用户要：指标名称后面括号内的参数也改为鼠标移到指标名上显示）：原 `indName(it)` 返回完整 `NAME(params)`（如 `MA(5,20)` / `MACD(12,26,9)` / 自定义 `MR_Y_MAIN(…)`）常驻显示。**修复（纯 JS）**：将 `indName` 拆为 `indBaseName(it)`（只返回基础名 MA/BOLL/MACD/KD/RSI，自定义返回 it.name）+ `indParams(it)`（返回原括号内参数字串），`indName` 保留为两者组合（向后兼容）；rowHtml 名称 span 改为 `<span … title="${indParams(it)}">${indBaseName(it)}</span>`→常驻只显基础名、悬停指标名即原生 tooltip 显示参数。**顺带修复一个既存 bug**：`inst` 对象未携带 `name`，而 `refreshIndLabels`（改参后刷新标签）用 `indName(inst)`，自定义指标会渲染成 `undefined(…)`；现给 `inst` 加 `name: it.name`，且 `refreshIndLabels` 改为同时置 `textContent=indBaseName(inst)` 与 `title=indParams(inst)`。与 v9.51（字段值悬停示参数名）一致用原生 title（不受 .legend-row overflow:hidden 裁剪）。node 内联脚本语法校验通过。待用户人工验证（图例指标名只剩 MA/MACD/MR_Y_MAIN 等基础名、悬停名称弹出参数 / 改参后名称与提示同步更新且自定义指标不再出现 undefined / 齿轮与字段值悬停提示仍正常）。未提交 git |
| 2026-09-23 | v9.53 | **图例指标名参数提示加括号 + 字段值字重与名称一致**（用户要：指标参数加上括号就行，另外字体大小和指标名称一致）：① 指标名悬停提示的参数字串加括号：v9.52 的 `title="${indParams(it)}"`（如 `5,20`）改为 `title="(${ps})"`（如 `(5,20)`）；buildInstance 预计算 `nameTip = ps ? `(${ps})` : ''`（空参不留空括号），`refreshIndLabels` 同步改为 `el.title = ps ? `(${ps})` : ''`。② 字段值字重：图例名与值本来同为 12px（均继承 .legend font-size），但值用 `<b>` 加粗、名为常规字重，加粗看起来比名称“大”；将值元素由 `<b id=…>` 改为普通 `<span id=…>`→字重与指标名一致（也与 OHLC/VOL 的 span 一致）；值元素始终按 id 访问（legendVals/refreshLegend/applyInstStyles 的 parentElement 均不依赖标签名），无 CSS/JS 选择器依赖 `b` 标签，改动安全。node 内联脚本语法校验通过（含无残留 `<b id="lg-`、无 querySelector('b') 核实）。待用户人工验证（悬停指标名显示带括号参数如 (5,20) / 字段值不再加粗、与指标名大小字重一致 / 改参后提示括号同步）。未提交 git |
