...
| IND-103 | 前端接入改造 | lc-live.html 删除 JS 指标计算函数，改为消费 REST 历史 + WS 增量；参数调整弹窗确认后改为重新请求后端；正式版 Vue 前端跟进替换 IndicatorPanel JS 计算；**指标增删交互**：工具栏 + 按钮弹出选择面板（主图/副图分组）添加指标，齿轮弹窗内置"移除指标"按钮，副图移除时销毁 pane 回收高度；允许同指标多参数实例（如 MA7+MA25）；`indParams` 固定键集合升级为 **indLayout 有序列表**（[{name, params}...]），四份用途同一结构：localStorage 布局持久化 / REST·WS 订阅入参 / 策略 require_indicators 入参 / 参数弹窗渲染源 | P0 |
